...
export default function Tasks() {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (editing) {
      ...
      addTask({ ...form, id: newId(), createdAt: today });
    }
    setShowForm(false);
  }
  ...
  // Resolve related name options based on relatedTo
  function getRelatedOptions() {
    if (form.relatedTo === 'Client')    return clients.map(c => ({ id: c.id, name: c.name }));
    if (form.relatedTo === 'Lead')      return leads.map(l => ({ id: l.id, name: l.title }));
        ...
        <Modal title={editing ? 'Edit Task' : 'Add Task'} onClose={() => setShowForm(false)}>
          <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input type="date" className="input" value={form.assignedDate}
